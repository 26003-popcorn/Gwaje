...
def inference(image_path, brightness, shifts):
    ...
    # 💡 [핵심 해결] 두 개의 모델 가중치를 각각 불러옵니다!
    model.load_state_dict(torch.load("best_model.pth", map_location=device))
    structure_model.load_state_dict(torch.load("best_structure.pth", map_location=device))
    ...
    mask_resized = cv2.resize(mask_sel, (256, 256))
    ...
    mask_full  = cv2.resize(mask_sel, (w, h))
